...
plots_tgd <- sm_put_together(all_plots, title=title, xlabel=xlabel,
...
plots_tgd2 <- sm_put_together(all_plots2, title=title, xlabel=xlabel,
                              ylabel=ylabel, ncol=3,nrow=3,
                              hmargin=-5, wmargin=-5)
...
1. `wRatio`: controls the relative width of the panels in the left-most column of the composite plot with ticks for the **primary y-axis**. This is automatically optimized as default if the user does not supply input (update made on **2024-05-24** available in the GitHub version).

2. `wRatio2`: controls the relative width of the panels in the right-most column of the composite plot with ticks for the **secondary y-axis**. This is automatically optimized as default if the user does not supply input (update made on **2024-05-24** available in the GitHub version).

3. `hRatio`: controls the relative height of the panels in the bottom row of the composite plot with ticks for the **primary x-axis**. This is automatically optimized as default if the user does not supply input (update made on **2024-05-24** available in the GitHub version).

4. `hRatio2`: controls the relative height of the panels in the top row of the composite plot with ticks for the **secondary x-axis**. This is automatically optimized as default if the user does not supply input (update made on **2024-05-24** available in the GitHub version).

5. `tickRatio`: controls the relative text size of the tick labels on both axes of the outer panels in the composite plot. This is automatically optimized as default if the user does not supply input (update made on **2024-05-24** available in the GitHub version).

It is possible that the optimization is not perfect because the current algorithm is quite simple and does not encompass all possible plotting situations. **So, users are encouraged to modify these arguments using values from 1.1 to 1.4**. 
...
*smplot2* offers multiple options for users to add annotations on the final, combined figure with a full flexibility. Here are some examples. These functions all have the form of `sm_add_*()`. Except for `sm_add_legend()`, all of these functions are added modularly to the composite plot (output from `sm_put_together()`).
...
```{r}
plots_tgd4b <- plots_tgd4 + sm_add_text(label = 'My composite figure',
...
```{r}
plots_tgd4c <- plots_tgd4b + sm_add_point(x = 0.50, y = 0.50, size = 35, 
...
plots_tgd4e <- plots_tgd4c +
  annotate('segment', x = 0.5, y = 0.475, xend = 0.5, 
                           yend = 0.525, color = 'black') +
  annotate('segment', x = 0.49, y = 0.5, xend = 0.51, 
