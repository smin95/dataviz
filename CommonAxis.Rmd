...
Each panel variable has to be included in a `list`. X-axis label can be specified using `sm_common_xlabel()`, which has `x` and `y` arguments for setting the location of the label. Y-axis label is specified using `sm_common_ylabel()` with the same `x` and `y` arguments. The title can be set using `sm_common_title()`. 
...
=======
xlabel <- sm_common_xlabel('Minutes after monocular deprivation', x= 0.52,
                           y = 0.6)
>>>>>>> eb1105960070a57adb98513ebcc1857b97e85ab3
...
=======
                             ncol=3,nrow=3, margin=0)

```

Now let's put them together in a 3x3 figure (3 rows, 3 columns) using the function `sm_put_together()`. So, `ncol` and `nrow` are set to 3 in `sm_put_together()`. The function automatically removes tick labels in both x- and y-axes in inner panels, and keeps them on the outer panels, so that combined plot looks clean. Also, the `margin` argument can be set to adjust the blank space between sub-panels.
>>>>>>> eb1105960070a57adb98513ebcc1857b97e85ab3
...
When you are saving the graph as an image file, `nrow` and `ncol` in `save_plot()` have to match the values in `sm_put_together()` as shown above.
...
You can even make a separate legend on your own with full customization using `sm_common_legend()`, then add the legend to the combined plot using `sm_add_legend()`.


```{r}
=======
Open `together1.png` in your directory folder. The figure is clean but it lacks a common legend. We can add a legend in a similar figure with 2x5 structure (`nrow=2` and `ncol=5`). The legend can be on the 10th panel.

```{r}
ylabel2 <- sm_common_ylabel('\u0394 Contrast balance ratio (dB)', x = 0.7)
...
Notice that the relative orientation of the legend has shifted. The `legend` can then be added to the combined plot using `sm_add_legend()`. If you provide a separate legend, then other arguments of the function will be ignored, such as `direction` and `border` from `sm_add_legend()`. 
...
plots_tgd4 <- sm_put_together(all_plots, title, xlabel,ylabel, legend,
                             ncol=5,nrow=2, hmargin=-5, wmargin=-5)

```

```{r}
save_plot('together3.png', plots_tgd4, ncol=5, nrow=2,
=======
  sm_common_legend(x = 0.5, y = 0.5 , title=FALSE, legend_spacing = 1) 

plots_tgd_legend <- sm_put_together(all_plots, title, xlabel, 
                                    ylabel= ylabel2, legend=legend, ncol=5,
                                    nrow=2, margin=0)
```
...
If the `legend` is included in `sm_put_together`, the function will automatically add legend onto the last panel of the combined plot, so make sure that the last empty panel is empty.


```{r}
save_plot('together2.png', plots_tgd_legend, ncol=5, nrow=2,
>>>>>>> eb1105960070a57adb98513ebcc1857b97e85ab3
          base_height=3.4, base_width=3)
```
...
<<<<<<< HEAD
knitr::include_graphics("together3.png")
```
=======
knitr::include_graphics("together2.png")
```

An astute reader will realize that the codes are quite repetitive. Could the codes be shortened? The answer is yes! However, this method is quite complex and beyond the scope of the guide.
