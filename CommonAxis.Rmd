...
The `title`, `xlabel`, `xlabel2`, `ylabel` and `ylabel2` can be provided as string characters, as shown below. If characters are provided rather than the outputs of `sm_common_title()`, `sm_common_xlabel()` and `sm_common_ylabel()`, then their locations and font sizes cannot be modified. These are instead estimated by a simple algorithm that relies on the plot's layout and other information. 
...
```{r}
save_plot('together4_quick.png', plots_tgd_quick, ncol=5, nrow=2,
          base_height=3.4, base_width=3)
...
These inputs are automatically optimized as default if the user does not supply input (available in version 0.2.2.2 from GitHub as of **2024-05-24**).
...
However, is possible that the optimization is not perfect because the current algorithm is quite simple and does not encompass all possible plotting situations. **So, users are encouraged to modify these arguments using values from 1.1 to 1.4**. 
...
In this short example, I recreate the composite plot generated above using the function `lapply()`, which creates an iterative loop, producing one ggplot2 object per loop. There will be nine loops total, and these will be stored in the output object `all_figs`. 
...
```{r}
plots_tgd5 <- sm_put_together(all_figs, title='Individual data', 
...
```{r, echo=FALSE}
save_plot('together5.png', plots_tgd5, ncol=5, nrow=2,
          base_height=3.4, base_width=3)
```
