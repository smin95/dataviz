...
However, is possible that the optimization is not perfect because the current algorithm is quite simple and does not foresee all possible plotting situations. **So, users are encouraged to modify these arguments using values from 1.1 to 1.4**. 
...
figs_mosaic <- lapply(1:numPanels, function(iPlot) {
  if (iPlot %% 2 == 0) {
    ggplot(NULL) + sm_common_legend() # empty panel
  } else if (iPlot %% 2 == 1) {
    i <-  iPlot %/% 2 + 1
    df_subj <- df %>% filter(Subject == subj_list[[i]])
    ggplot(data = df_subj, aes(x = Time, y = Cbratio, color = Day)) + 
...
```{r, warning=F, message=F}
mosaic_tgd <- sm_put_together(figs_mosaic, title='Mosaic of individual data', 
                              xlabel='Minutes after monocular deprivation',
...
```{r, warning=F, message=F}
save_plot('mosaic.png', mosaic_tgd1, ncol=ncol, nrow=nrow,
          base_height=3.4, base_width=3)
```
