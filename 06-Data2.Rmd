...
# Data Analysis Part 2 (in progress)
...
In Chapter 5 of the online guide, we only dealt with data from Days 1 and 5. In this chapter, we will use data from Days 1, 2, 3, 4 and 5. 
...
## More Statistics (ex. Anova)

The reader may wonder what the asterisks indicate in Figure 3E. These denote statistically significant difference between the AUC from each day and 0. This can be computed using **one-sample t-test**.

The reader may be curious as to how she can determine whether the AUCs across five days significantly differ. In short, how can one analyze if the effect of one variable (i.e., Day) significantly influences the data. If the AUCs significantly differ across days, then the results might be more interesting. This could be achieved by using a **one-way analysis of variance (ANOVA)**.

The reader may also question how I have compared each point as shown in Figure 3B across different time points and days. In short, how can one determine whether the effects of two variables (ex. Day and Time after patching) significantly influence the data? This could be done using a **two-way analysis of variance (ANOVA)**. 
