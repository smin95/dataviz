...
- `wRatio`, `wRatio`, `hRatio`, `hRatio2` and `tickRatio` of `sm_put_together()` are adjusted automatically unless users supply their own inputs.

-  Defaults of visualization functions are now kept even when users call upon `list()` for each argument unless users directly replace them with another input value. For example, to change the transparency of the bar and keep the defaults of `sm_bar()`, users can only re-specify the alpha value as shown below.

  - To forget the defaults for a plot component (ex. bar), set `forget = TRUE` and call `list()` for the component, and the visualization functions **will work the same as before** the version update.
...
ggplot(data = df, mapping = aes(x = Day, y = Value, color = Day)) +
  sm_bar(bar.params = list(alpha = 0.3), forget = TRUE) + 
  scale_color_manual(values = sm_color('blue','orange')) +
