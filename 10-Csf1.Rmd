...
We will use data of achromatic contrast sensitivity from 10 normal observers from this paper (a subset of the entire dataset):
...
- First, `Subject` refers to each participant. There are 10 participants total in the .csv file.
...
First, let's compute the average and standard errors using the dataset. It is recommended that the reader has read Chapters 5 and 6 already. Understanding of `summarise()`, `filter()`, `mutate()` and `%>%` are assumed.

The code below reorganizes the `ACh` dataset so that instead of containing the data of each subject, it now contains the averaged data across `Repetition` and `SpatialFreq` as well as their standard errors and standard deviations.
...
ACh_avg1 %>% ggplot(aes(x = SpatialFreq, y = avgSens)) +
  ...
  ggtitle('Repetition 1 (n=10)')
```

So far, this graph does not show the contrast sensitivity function, which as a rotund shape. Instead, if we connect the points, we get this plot instead.
...
This plot shows the averaged sensitivity for each spatial frequency during the second repetition. However, the contrast sensitivity function is still missing because this plot instead shows the plot that is connected between a pair of the points. To plot the contrast sensitivity function with the data, `sm_CSF()` is required.
...
ACh_avg2 %>% ggplot(aes(x = SpatialFreq, y = avgSens)) +
  ...
  xlab('Spatial frequency (c/deg)') +
  ylab('Averaged sensitivity') +
  ggtitle('Repetition 2 (n=10)') 
```

Notice that `sm_CSF()` is written first because the curve has to appear before the points (`geom_point()`); else, the points will be covered the CSF curve. This example is shown below.
...
To plot the linear CSF and its area, `logXY = FALSE` has to be set `FALSE` in both `sm_areaCSF()` and `sm_CSF()`.
...
The **smCSF** package also offers a function that draws a ribbon shade to denote standard error or standard deviation. This function works exactly the same as `geom_ribbon()`. It requires the uses to specify `ymin` and `ymax`. The example below plots standard error using `sm_ribbonCSF()`.
...
ACh_avg1 %>% ggplot(aes(x = SpatialFreq, y = avgSens)) +
  sm_ribbonCSF(aes(ymin = avgSens - stdDevSens,
                   ymax = avgSens + stdDevSens),
               fill = sm_color('skyblue'), alpha = 0.4) +
...
To facet means to split the plot into multiple plots based on a certain variable. We will facet the contrast sensitivity function plot above (blue curve) into two panels, each of which represents each repetition (red and green).
