...
- `sm_statCorr()` draws the fitted linear regression line, prints the `R`  (or R2) and `p` values from the specified `method` of correlation test. It also calls `sm_hvgrid()`.
...
- `separate_by` is the string that separates between the `R` (or R2) print and the `p` print.
