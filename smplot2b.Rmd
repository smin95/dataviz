...
`sm_slope~mean()` plots a slope chart with mean. It only visualizes data in one group and is less flexible than `sm_slope()`. However, it is much easier to draw the mean slope chart than `sm_slope()`. Here is a simple recreation of the slope chart from the previous section. 
