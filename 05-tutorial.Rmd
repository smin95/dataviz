...
This chapter is for those who know how to use ggplot2 already and skipped Chapters 1-3 from the online tutorial.
...
- In this case, the error bar represents **standard error**. If you prefer to show **standard deviation**, then you should set `se = FALSE` in `sm_bar()`.
  
  
```{r,fig.width = 3.7, fig.height=3.7,warning = F}
ggplot(data = df, mapping = aes(x = Day, y = Value, fill = Day)) +
  sm_bar(shape = 21, color = 'white', bar_fill_color = 'gray80', se = FALSE) +
  scale_fill_manual(values = sm_color('blue','orange'))
```
...
- So far the distribution plots (violin plots) have been vertical. We can change their location by setting `vertical = FALSE`. 
