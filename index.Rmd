...
**smplot2** has been developed from **smplot**. The updates are too big and drastic to keep everything in the original **smplot**.
...
Also, if you are capable of adapting to some changes made to **smplot2**, **please delete smplot** and then use **smplot2**. More information is on Chapters 11 and 12.
...
It turns out that the most recent version of ggplot2 (3.4.0) has some issues with `sm_raincloud()`. 

```{r}
packageVersion('ggplot2')
```

If your package version is `3.4.0`, please download the version `3.3.6` of ggplot2.

```{r, eval = FALSE}
g <- 'http://cran.r-project.org/src/contrib/Archive/ggplot2/ggplot2_3.3.6.tar.gz'
install.packages(g, repos=NULL, type="source")
