...
**Note:** We will use `rstatix` to perform ANOVA. However, both `tidyverse` and `rstatix` packages have the function called `filter()`. To use  `filter()` from the `tidyverse` (or `dplyr`) package, as we have done so before, you will need to type `dplyr::filter` rather than `filter` instead to specify `filter()` from the `dplyr` package.
...
- Homogeneity of variance. The variance of the data should be consistent as a function of the predictor (x-axis). This is beyond the scope of this book and therefore not covered.
...
If one-way ANOVA shows a significant difference (p < 0.05), then the reader should perform **Tukey HSD** post-hoc pairwise comparison tests to see which two specific groups vary significantly. In the example, I have saved the results in the variable `res` (short for results).

```{r}
res <- auc_df %>% tukey_hsd(AUC_Cbratio ~ Day)
res
```

As expected from one-way ANOVA which shows no significant difference across days, the post-hoc pairwise tests (Tukey HSD test) yield p-values higher than 0.05.
...
If the subjects are paired across the variable (or `Day`), then the arguments of `anova_test()` might need to be specified as follows.
...
If different subjects were tested on each day, then `wid` should be empty. In this example, regardless of whether `wid` is filled in `anova_test()`, the p-value is the same because the subject identifiers (ex. `S1`, `S2`) are unique to each subject.

## Two-way ANOVA

In a two-way ANOVA, the effects of two factors (or variables) are examined. In our example, we will use the data set from the paper, and examine the effects of `Day` and `Time`, which is the time point after short-term monocular deprivation (minutes).
...
As before, when ANOVA is computed using `anova_test()`, `group_by()` is not necessary because you are not summarising raw data into average or median etc.

```{r}
df %>% anova_test(Cbratio ~ Day * Time)
```

The `*` represents the statistically significant of `Time`. However, it is absent for the effects of `Day` and the interaction between `Day` and `Time`. 

It appears that there is a statistically significant effect of `Time`, F(1, 290) = 30.11, p < 0.001. But the effect of `Day` seems to be not significant, F(4,290) = 0.96, p > 0.05. Also the effect of interaction between `Day` and `Time` is not significant, F(4,290) = 0.31, p > 0.05. 